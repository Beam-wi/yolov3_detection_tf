...
def getDataSet(graph, detTrainPath, detValPath, nfsMountDir, dataTempDir,
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        gpuNmsOp = gpu_nms(predBoxesFlag, predLabelsFlag, predScoresFlag, classNum)
...
def main():
    ...
    qjClassPath = os.path.join(detDataPath, "qj_class.txt")
    ...
    with open(qjClassPath, "w", encoding='utf-8') as af:
        af.write("qj")
    ...
    anchors = np.reshape(
        np.asarray(cfg.data_set["anchors"], np.float32), [-1, 2]) * \
              cfg.data_set["anchorRotate"]
