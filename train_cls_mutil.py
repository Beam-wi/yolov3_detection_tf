...
from utils.misc_utils import shuffle_and_overwrite, average_gradients
...
def getDataSet(graph, clsFileTrain, clsFileVal, classNameDict, cfg):
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        # build Dataset
        shuffle_and_overwrite(clsFileTrain)  # 每轮都随机打乱数据集顺序
...
def getModel(graph, gpus, datasetIterator, trainIterator, trainHandle,
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        with tf.device(gpus[0]):
                ...
                for i, gpu in enumerate(gpus):
                    with tf.device(gpu):
                        with tf.name_scope('%s_%d' % ('tower', i)) as scope:
                            image, yTrue, y = datasetIterator.get_next()
                            image.set_shape([None, cfg.model_set["classify_size"][1], cfg.model_set["classify_size"][0], 3])
                            yTrue.set_shape([None, len(classNameDict.keys())])
                            y.set_shape([None, ])
                            ...
                            grads = optimizer.compute_gradients(lossAll)
                            towerGrads.append(grads)
                            # towerLoss.append(lossAll)

                            correctPrediction = tf.equal(
                                tf.argmax(logits, 1), tf.argmax(yTrue, 1))
                            accuracy = tf.reduce_mean(
                                tf.cast(correctPrediction, tf.float32))

            meanGrads = average_gradients(towerGrads)
            updateOps = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            with tf.control_dependencies(updateOps):
                trainOp = optimizer.apply_gradients(meanGrads)
            ...
            return sess, cost, trainOp, accuracy, centerLoss, distance, lossAll, \
                   trainHandleValue, valHandleValue, isTraining, saver, logits, yTrue
...
def train(sess, cfg, cost, trainOp, accuracy, centerLoss, distance, lossAll,
          ...
          isTraining, logger, saver, logits, yTrue):
    ...
    for epoch in range(1, cfg.model_set["classify_total_epochs"] + 1):
        ...
        for step in range(1, trainBatchNum):
            trainFeedDict = {
                isTraining: True,
                handleFlag: trainHandleValue,
                learningRate: epochLearningRate}
            # batchLoss, _, batchAcc, batchCenterLoss, distance_, lossAll_, logits_, yTrue_ = \
            #     sess.run(
            #         [cost, trainOp, accuracy, centerLoss, distance, lossAll, logits, yTrue],
            #         feed_dict=trainFeedDict)

            # batchLoss, _, batchAcc, batchCenterLoss, lossAll_, logits_, yTrue_ = \
            #     sess.run(
            #         [cost, trainOp, accuracy, centerLoss, lossAll, logits, yTrue],
            #         feed_dict=trainFeedDict)

            batchLoss, _, batchAcc, lossAll_, logits_, yTrue_ = \
                sess.run(
                    [cost, trainOp, accuracy, lossAll, logits, yTrue],
                    feed_dict=trainFeedDict)
        ...
        if valNum != 0:
            ...
            #     saver.save(
            #         sess=sess,
            #         save_path=f'{modelSavePath}/{cfg.model_set["classify_model_save_name"]}-{epoch}')
            #     logger.info(f'{modelSavePath}/{cfg.model_set["classify_model_save_name"]}-{epoch}')
        builddata_shuffle_and_overwrite(
...
def main():
    ...
    detValPath = detTrainPath.replace('_train.txt', '_val.txt').replace('_train_or.txt', '_val.txt')
    ...
    try:
        ...
        trainBatchNum = int(np.ceil(float(trainNum) / (batchSize*len(gpus))))
        valBatchNum = int(np.ceil(float(valNum) / (batchSize*len(gpus))))
        ...
        datasetIterator, trainIterator, trainHandle, valHandle, classNameDict, \
        ...
        trainHandleValue, valHandleValue, isTraining, saver, logits, yTrue= \
            getModel(graph, gpus, datasetIterator, trainIterator, trainHandle,
                     valHandle, classNameDict, learningRate, isTraining, cfg, logger)
